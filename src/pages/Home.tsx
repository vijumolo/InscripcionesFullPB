...
import { createParticipant, fetchLatestEvent } from '../lib/api';
...
export const Home = () => {
    ...
    const fetchEventDetails = async () => {
        try {
            ...
            setFetchError(error instanceof Error ? error.message : 'No fue posible cargar el evento desde PocketBase.');
        } finally {
            setLoading(false);
        }
    };

    const onSubmit = async (data: ParticipantPayload) => {
        if (!eventDetails) return;
        setSubmitting(true);
        setSubmitError(null);

        try {
            const participantData: ParticipantPayload = {
                ...data,
            ...
            setRegisteredData(participantData as Participant);
            setSuccess(true);
        } catch (error: any) {
        ...
        return (
            <div className="text-center py-12 glass rounded-2xl">
                ...
                <p className="mt-2 text-slate-600">
                    {fetchError || 'Por el momento no tenemos eventos disponibles para inscripción.'}
                </p>
            </div>
        );
    }
